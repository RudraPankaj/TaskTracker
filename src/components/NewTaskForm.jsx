...
import { BsXCircleFill, BsPlusCircleFill, BsFileText } from 'react-icons/bs'; // Import Bootstrap Icons
...
export default function NewTaskForm({ onAdd }) {
  ...
  return (
    <div className="bg-gray-900 p-8 rounded-lg shadow-lg w-full h-full text-white overflow-auto">
      ...
      <form onSubmit={submit} className="space-y-6 h-full flex flex-col">
        ...
        <div className="mb-6 flex-1">
          <div className="flex items-center gap-4 mb-2">
            <button
              ...
              className="text-xs bg-blue-700 hover:bg-blue-800 text-white px-2 py-1 rounded-md flex items-center gap-1"
            >
          ...
          <div className="mt-4">
            <label className="text-sm text-gray-400 mb-2 block">Add Multiple Subtasks (one per line)</label>
            <textarea
              value={plaintextSubtasks}
              onChange={(e) => setPlaintextSubtasks(e.target.value)}
              className="w-full p-3 bg-gray-800 rounded-lg border border-gray-700 focus:outline-none focus:border-blue-500 h-24"
              placeholder="e.g.,&#10;Subtask 1&#10;Subtask 2"
            ></textarea>
            <button
              type="button"
              onClick={addPlaintextSubtasks}
              className="mt-2 text-xs bg-purple-700 hover:bg-purple-800 text-white px-2 py-1 rounded-md flex items-center gap-1"
            >
              <BsFileText className="inline-block text-base" /> Add from Text
            </button>
          </div>
          <div className="mt-4">
            {subtasks.map((sub) => (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full p-2 bg-gray-800 rounded-lg border border-gray-700 focus:outline-none focus:border-blue-500"
            />
          </div>
          <div>
        ...
        <div className="flex gap-4 pt-4">
          <button
            type="submit"
            className="w-full px-6 py-3 bg-green-600 rounded-lg hover:bg-green-700 transition-colors font-semibold flex items-center gap-2 justify-center"
          >
